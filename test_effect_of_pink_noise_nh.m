...
nh_data_dir= '/media/parida/DATAPART1/Matlab/ExpData/MatData/SP-2018_11_08-Q369_SFR_NH_3/';
chinID= 369;
...
allfiles= dir([nh_data_dir 'a*SFR*.mat']);
nTypes= {'HP', 'LP'};
    ...
    for nTypeVar= 1:length(uniq_nType)
        ...
        if ~isempty(cur_inds)
            ...
            for fileVar= 1:length(cur_inds)
                ...
                temp_data= load([nh_data_dir allfiles(fileInd).name]);
            ...
            figHan= 1;
            fName= strrep(sprintf('Q%d_nh_SNR%d_%s', chinID, curSNR, cur_nType), '-', 'm');
